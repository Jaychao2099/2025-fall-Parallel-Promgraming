...
static __inline __attribute__((__gnu_inline__, __always_inline__))
void mandelbrot_serial(WorkerArgs *const args)
{
    ...
    int *__restrict__ output = (int *)__builtin_assume_aligned(args->output, 32);
    ...
    // -------------------- 每 pixel 交錯執行 ---------------------------------
    for (int k = threadId; k < image_size; k += num_threads) {
        const float y = y0 + ((float)(k/width) * dy);
        const float x = x0 + ((float)(k%width) * dx);
        output[k] = mandel(x, y, max_iterations);
    }
}
