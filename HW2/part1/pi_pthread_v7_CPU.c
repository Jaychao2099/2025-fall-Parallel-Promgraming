...
#include <sched.h>
#include <unistd.h>   // 為了 sysconf(_SC_NPROCESSORS_ONLN)
...
static inline __m256i rand_vec(prng_state *s) {
    // xorshift+ is fast and provides good quality random numbers.
...
void *toss(void *args) {
    ...
    *local_number_in_circle = 0LL;
    ...
    for (i = 0; i < total_tosses_in_vec; i += SIMD_WIDTH) {
        // --- AVX2-compatible random double generation in [0, 1) ---
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < thread_cnt; i++) {
        ...
        cpu_set_t cpuset;
        CPU_ZERO(&cpuset);  // 空集合
        CPU_SET(i % num_cores, &cpuset); // i 是執行緒編號, num_cores 是你的 CPU 核心數
